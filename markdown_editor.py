...
class MarkdownEditor:
    ...
    def setup_ui(self):
        ...
        self.toolbar = tk.Frame(self.root, bg=self.themes[self.current_theme]["toolbar_bg"])
        ...
        # Метка редактора
        editor_label = tk.Label(self.editor_frame, text="Редактор Markdown",
        ...
        # Метка предпросмотра
        preview_label = tk.Label(self.preview_frame, text="Предпросмотр",
                                bg=self.themes[self.current_theme]["toolbar_bg"],
                                fg=self.themes[self.current_theme]["fg"])
        ...
        # Строка состояния
        self.statusbar = tk.Frame(self.root, bg=self.themes[self.current_theme]["toolbar_bg"], height=25)
        ...
        # Счётчик символов
        self.char_count_label = tk.Label(self.statusbar, text="Символов: 0",
                                        anchor=tk.E,
                                        bg=self.themes[self.current_theme]["toolbar_bg"],
                                        fg=self.themes[self.current_theme]["fg"])
        ...
        # Счётчик слов
        self.word_count_label = tk.Label(self.statusbar, text="Слов: 0",
                                        anchor=tk.E,
                                        bg=self.themes[self.current_theme]["toolbar_bg"],
                                        fg=self.themes[self.current_theme]["fg"])
    ...
    def create_format_buttons(self):
        ...
        theme_btn = tk.Button(self.toolbar, text="Тема", command=self.toggle_theme,
                             bg=self.themes[self.current_theme]["toolbar_bg"],
                             fg=self.themes[self.current_theme]["fg"],
                             padx=5, pady=3)
    ...
    def setup_menu(self):
        ...
        edit_menu.add_separator()
        edit_menu.add_command(label="Найти и заменить", command=self.find_replace, accelerator="Ctrl+F")
        
        # Вид
        view_menu = tk.Menu(menubar, tearoff=0)
        menubar.add_cascade(label="Вид", menu=view_menu)
        view_menu.add_command(label="Обновить предпросмотр", command=self.update_preview)
...
<head>
    ...
    <style>
        body {{
            ...
            margin: 0 auto;
            padding: 20px;
            {'color: #e0e0e0; background-color: #1e1e1e;' if self.current_theme == 'dark' else 'color: #000000; background-color: #ffffff;'}
        }}
        h1, h2, h3, h4, h5, h6 {{
            {'color: #444444;' if self.current_theme == 'dark' else 'color: #333333;'}
        }}
        code {{
            {'background-color: #3d3d3d; color: #d4d4d4;' if self.current_theme == 'dark' else 'background-color: #f4f4f4; color: #d14;'}
            padding: 2px 6px;
            border-radius: 3px;
        ...
        blockquote {{
            {'border-left: 4px solid #555;' if self.current_theme == 'dark' else 'border-left: 4px solid #ccc;'}
            margin: 0;
            padding-left: 16px;
            {'color: #aaa;' if self.current_theme == 'dark' else 'color: #666;'}
        }}
        table {{
        ...
        th, td {{
            border: 1px solid {'#555;' if self.current_theme == 'dark' else '#ddd;'}
            padding: 8px;
            text-align: left;
        }}
        th {{
            {'background-color: #444;' if self.current_theme == 'dark' else 'background-color: #f2f2f2;'}
        }}
    </style>
    <!-- MathJax для LaTeX -->
...
{html}
    ...
    def process_latex(self, html):
        """Обработка LaTeX-формул в HTML"""
        # Замена блочных формул $$...$$ на MathJax блоки
        # Замена встроенных формул $...$ на MathJax инлайн
        # Use a more robust approach to handle nested brackets
        import re
        
        # Process display math first (to avoid conflicts with inline math)
        # This regex is simplified and might need improvements for complex cases
        def replace_display_math(match):
            content = match.group(1)
            # Escape special characters in the content for HTML
        ...
        # Look for $$...$$ but be careful about escaping
        # This pattern looks for $$ followed by any characters (non-greedy) until $$
        html = re.sub(r'\$\$(.*?)\$\$', replace_display_math, html, flags=re.DOTALL)
        
        # Process inline math
        # Look for $...$ but not $$...$$
        # This is tricky because we need to avoid matching $$...$$ again
        # We'll use a negative lookbehind and lookahead to avoid $$...$$
        # But this is complex, so let's use a simpler approach: process $...$ but skip those preceded by $
    ...
    def insert_text(self, text):
        ...
        try:
            ...
            if '$' in text and text.count('$') == 1:  # For single $ insertion (LaTeX inline)
    ...
    def insert_image(self):
        ...
        if filepath:
            alt_text = simpledialog.askstring("Вставить изображение", "Альтернативный текст:")
            if alt_text is None:
    ...
    def export_to_pdf(self):
        """Экспорт в PDF"""
        if not WeasyHTML:
            messagebox.showerror("Ошибка", "Библиотека weasyprint не установлена.\nУстановите: pip install weasyprint")
            return
    ...
    def find_replace(self):
        ...
        # Поле поиска
        tk.Label(find_window, text="Найти:").grid(row=0, column=0, padx=5, pady=5, sticky="e")
        find_entry = tk.Entry(find_window, width=30)
        find_entry.grid(row=0, column=1, padx=5, pady=5)
        
        # Поле замены
        tk.Label(find_window, text="Заменить на:").grid(row=1, column=0, padx=5, pady=5, sticky="e")
        ...
        def replace_all():
            ...
            if search_text:
                ...
                self.editor.delete("1.0", tk.END)
                self.editor.insert("1.0", new_content)
                messagebox.showinfo("Замена", f"Заменено {content.count(search_text)} вхождений")
        
        tk.Button(find_window, text="Найти", command=find_next).grid(row=2, column=0, padx=5, pady=5)
        tk.Button(find_window, text="Заменить", command=replace).grid(row=2, column=1, padx=5, pady=5)
        tk.Button(find_window, text="Заменить все", command=replace_all).grid(row=2, column=2, padx=5, pady=5)
...
- WeasyPrint"""
    ...
    def quit(self):
        """Выход из приложения"""
        if self.is_dirty:
            result = messagebox.askyesnocancel("Сохранить?", "Сохранить текущий файл перед выходом?")
            if result:  # Да
                self.save_file()
            elif result is None:  # Отмена
                return
    ...
    def load_last_session(self):
        """Загрузка последней сессии"""
        session_file = os.path.join(tempfile.gettempdir(), "markdown_editor_session.json")
...
if __name__ == "__main__":
    ...
    def insert_text(self, text):
        ...
        try:
            selection = self.editor.selection_get()
            self.editor.insert(tk.INSERT, text + selection + text.replace('$', '') if '$' in text else '')
        except tk.TclError:
            self.editor.insert(tk.INSERT, text)
